#!/usr/bin/env python3
"""
18k_full_hko_contract_event_target_panel_FIX2.py
...
def md_table(df: pd.DataFrame, cols: Optional[List[str]] = None, max_rows: int = 50) -> str:
    """Render a small markdown table robustly.

    Important: avoid DataFrame.fillna("") directly on pandas nullable integer
    columns, because recent pandas versions raise TypeError for Int64 arrays.
    Convert to object first, then replace missing values with blank strings.
    """
    if df.empty:
        return "_No rows._"
    d = df.copy()
    if cols is not None:
        ...
        d = d[cols]
    d = d.head(max_rows).astype(object)
...
def main() -> None:
    ...
    # Important repair:
    # The 18j v2 universe may already contain a blank metadata column named
    # hko_tmax_C. After the merge, the realised HKO value from the target table is
    # therefore held in hko_tmax_C_hko. We must use the joined realised value for
    # payoff computation, while keeping the original metadata value for audit.
    if "hko_tmax_C_hko" in panel.columns:
        panel["hko_tmax_C_original_18j"] = pd.to_numeric(panel.get("hko_tmax_C"), errors="coerce")
        panel["hko_tmax_C"] = pd.to_numeric(panel["hko_tmax_C_hko"], errors="coerce")
    else:
        panel["hko_tmax_C"] = pd.to_numeric(panel.get("hko_tmax_C"), errors="coerce")
