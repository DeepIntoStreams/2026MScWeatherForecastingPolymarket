...
def find_repo_root() -> Path:
    """Return the repository root even when the script is launched from notebooks/."""
    candidates = [Path.cwd()]
    try:
    ...
    for candidate in candidates:
        ...
        for p in [candidate, *candidate.parents]:
            has_expected_input = (p / "data" / "processed" / "18j_gamma_metadata_settlement_family_audit.csv").exists()
            looks_like_repo = (p / ".git").exists() and (p / "data").exists()
            if has_expected_input or looks_like_repo:
                return p
...
def main() -> None:
    print(f"Repository root detected: {ROOT}")
    if not INPUT_AUDIT.exists():
        nearby = sorted((PROCESSED).glob("18j*")) if PROCESSED.exists() else []
        msg = [
            f"Missing input audit file: {INPUT_AUDIT}.",
            "The v2 repair needs the first 18j output to exist first.",
            f"Current working directory: {Path.cwd()}",
            f"Detected repository root: {ROOT}",
            "Nearby 18j files in data/processed:",
        ]
        msg.extend([f"  - {p.name}" for p in nearby[:40]])
        raise SystemExit("\n".join(msg))
